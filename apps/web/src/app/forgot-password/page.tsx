        ...
        <div className="rounded-3xl border border-neutral-200 bg-white p-7 shadow-[0_24px_70px_rgba(15,23,42,.08)] sm:p-9">
          {done ? (
            <div className="text-center">
              <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-2xl bg-brand-50 text-brand-700"><CheckCircle2 className="h-6 w-6" /></div>
              ...
              <Link href="/login" className="mt-7 inline-flex items-center gap-2 font-bold text-brand-700"><ArrowLeft className="h-4 w-4" /> Back to login</Link>
            </div>
          ) : (
            <>
              <div className="mb-7"><div className="mb-4 flex h-11 w-11 items-center justify-center rounded-2xl bg-brand-50 text-brand-700"><Mail className="h-5 w-5" /></div><h1 className="text-2xl font-bold">Reset your password</h1><p className="mt-2 text-sm leading-6 text-neutral-500">Enter your email and we&rsquo;ll send password reset instructions if an account exists.</p></div>
              <form onSubmit={submit} className="space-y-5">
                ...
                <button disabled={submitting} className="flex w-full items-center justify-center gap-2 rounded-2xl bg-brand-500 px-4 py-3.5 font-bold text-navy-950 transition hover:bg-brand-600 disabled:cursor-not-allowed disabled:opacity-60">{submitting && <LoaderCircle className="h-4 w-4 animate-spin" />}Send reset instructions</button>
              </form>
              <p className="mt-6 text-center text-sm text-neutral-500"><Link href="/login" className="font-bold text-brand-700">Back to login</Link></p>
            </>
          )}
        </div>
      </div>
