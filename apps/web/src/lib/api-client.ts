...
/** Authenticated request — attaches the bearer token; on a 401, refreshes the session once and retries before giving up. */
...
export async function logoutAll(): Promise<void> {
  try {
    await authedFetch<void>('/auth/logout-all', { method: 'POST' });
  } finally {
    clearSession();
  }
}
...
export function forgotPassword(email: string): Promise<{ message: string }> {
  return publicFetch('/auth/password/forgot', { method: 'POST', body: JSON.stringify({ email }) });
}

export function resetPassword(token: string, newPassword: string): Promise<void> {
  return publicFetch('/auth/password/reset', { method: 'POST', body: JSON.stringify({ token, newPassword }) });
}

export function verifyEmail(token: string): Promise<void> {
  return publicFetch('/auth/email/verify', { method: 'POST', body: JSON.stringify({ token }) });
}

export function requestPhoneOtp(phone: string): Promise<{ message: string }> {
  return authedFetch('/auth/phone/otp/request', { method: 'POST', body: JSON.stringify({ phone }) });
}

export function verifyPhoneOtp(phone: string, code: string): Promise<void> {
  return authedFetch('/auth/phone/otp/verify', { method: 'POST', body: JSON.stringify({ phone, code }) });
}
...
export function confirmTwoFactor(code: string): Promise<{ backupCodes: string[] }> {
  return authedFetch('/auth/2fa/confirm', { method: 'POST', body: JSON.stringify({ code }) });
}

export function disableTwoFactor(password: string): Promise<void> {
  return authedFetch('/auth/2fa/disable', { method: 'POST', body: JSON.stringify({ password }) });
}

/** Mirrors `RequestUser` in apps/api/src/authorization/types/request-user.ts exactly — this is what GET /auth/me actually returns (not `PublicUser` — it has no kycStatus/emailVerifiedAt/phoneVerifiedAt). */
...
export function getWallet(currency = 'USD'): Promise<WalletBalance> {
  return authedFetch(`/wallet?currency=${encodeURIComponent(currency)}`);
}
...
/** No server-side type/status filter exists on this endpoint (only limit/offset) — any type filtering must happen client-side over the fetched page. */
export function listWalletTransactions(params: { limit?: number; offset?: number } = {}): Promise<{ items: Transaction[] }> {
...
export function listInstruments(params: { category?: string } = {}): Promise<{ items: Instrument[] }> {
...
export function getPrice(instrumentId: string): Promise<PriceQuote> {
...
/** Mirrors `serializeBet` in `apps/api/src/betting/mappers.ts` exactly. */
...
export function getBettingConfig(instrumentId: string, type: BetType): Promise<BettingConfig> {
  return authedFetch(`/betting/configs?instrumentId=${encodeURIComponent(instrumentId)}&type=${type}`);
}
...
export function placeBet(input: PlaceBetInput, idempotencyKey: string): Promise<Bet> {
...
export function listBets(params: { status?: BetStatus; limit?: number; offset?: number } = {}): Promise<{ items: Bet[] }> {
  const query = new URLSearchParams();
...
export function createDeposit(input: MoneyAmountInput, idempotencyKey: string): Promise<Deposit> {
...
export function listDeposits(params: { status?: DepositStatus; limit?: number; offset?: number } = {}): Promise<{ items: Deposit[] }> {
  const query = new URLSearchParams();
  if (params.status) query.set('status', params.status);
  if (params.limit) query.set('limit', String(params.limit));
  if (params.offset) query.set('offset', String(params.offset));
...
export function cancelDeposit(id: string): Promise<Deposit> {
  return authedFetch(`/payments/deposits/${id}/cancel`, { method: 'POST' });
}
...
export function createWithdrawal(input: MoneyAmountInput, idempotencyKey: string): Promise<Withdrawal> {
...
export function listWithdrawals(params: { status?: WithdrawalStatus; limit?: number; offset?: number } = {}): Promise<{ items: Withdrawal[] }> {
  const query = new URLSearchParams();
  if (params.status) query.set('status', params.status);
  if (params.limit) query.set('limit', String(params.limit));
  if (params.offset) query.set('offset', String(params.offset));
