...
function makeHarness(
  overrides: { db?: Partial<DrizzleDb> } = {},
) {
  const db = {
    select: jest.fn(),
    insert: jest.fn(),
...
describe('PriceFeedService', () => {
  describe('ingestTick', () => {
    ...
    it('scales the decimal price using the instrument precision and returns an API-safe decimal string quote', async () => {
      ...
      const quote = await service.ingestTick(
        instrument(),
        {
    ...
    it('publishes the tick on the price stream for that instrument', async () => {
      ...
      const received = firstValueFrom(
        service.priceStream$('inst-btc'),
      );

      await service.ingestTick(
        instrument(),
        {
          priceDecimal: '1.00',
          source: 'test',
  ...
  describe('getLatestPrice', () => {
    it('throws NoPriceAvailableError when there has never been a tick', async () => {
      ...
      (
        instrumentService.getById as jest.Mock
      ...
      await expect(
        service.getLatestPrice('inst-btc'),
  ...
  describe('refreshFromProvider', () => {
    ...
    it('ingests the quote returned by the provider', async () => {
      ...
      (
        provider.getQuotes as jest.Mock
      ).mockResolvedValue([
        {
          providerSymbol: 'bitcoin',
          quoteCurrency: 'USD',
          priceDecimal: '112500.00',
          observedAt: NOW,
        },
      ]);
      ...
      expect(result?.price).toBe('112500.00');
    ...
    it('swallows a provider failure instead of throwing or fabricating a price', async () => {
      ...
      await expect(
        service.refreshFromProvider(instrument()),
  ...
  describe('refreshAllActive', () => {
    it('isolates missing provider quotes — instruments with returned quotes are ingested independently', async () => {
      ...
      (
        instrumentService.list as jest.Mock
      ).mockResolvedValue([
        instrument({
          id: 'a',
          providerSymbol: 'bitcoin',
      ...
      const insertedInstrumentIds =
        insertChain.values.mock.calls.map(
          ([values]: [{ instrumentId: string }]) =>
            values.instrumentId,
        );

      expect(
        insertedInstrumentIds.sort(),
      ).toEqual(['a', 'c']);
    });
  });
