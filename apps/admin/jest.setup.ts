...
// `useRouter`/`useSearchParams`/`useParams` don't throw — individual tests
// can still `jest.mock('next/navigation', ...)` with more specific
// behavior when the navigation itself is under test.
