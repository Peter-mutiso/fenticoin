...
export function InstrumentPrice({ instrumentId, currency, className }: { instrumentId: string; currency: string; className?: string }) {
  ...
  return <PriceQuoteBadge price={priceQuery.data} currency={currency} loading={priceQuery.isLoading} className={className} />;
}
